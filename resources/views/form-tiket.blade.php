...
<div x-data="pemesananApp()" x-init="loadOrderFromStorage()" class="container mx-auto max-w-6xl p-4 md:p-10">
  <div class="relative flex justify-center items-center mb-8">
    <a href="{{ route('tiket') }}" class="absolute left-0 text-gray-600 hover:text-indigo-800 transition-colors">
  ...
  <div class="lg:grid lg:grid-cols-5 lg:gap-12">
    <div class="lg:col-span-3">
      ...
      <p class="text-gray-600 mt-2 mb-8">Lengkapi formulir, lalu klik “Bayar Sekarang”.</p>
      ...
      <form @submit.prevent="submitPayment" class="space-y-8">
          ...
          <div @click="isPaymentOpen=!isPaymentOpen"
            ...
            <div class="flex items-center space-x-3">
              <template x-if="!selectedPayment">
                <svg class="w-6 h-6 text-indigo-700" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h6m3-3.75l-3 3m0 0l-3-3m3 3V15m6-1.5h.008v.008H18V15m-1.5 0h.008v.008H16.5V15"/></svg>
              </template>
              <template x-if="selectedPayment">
              ...
              <div>
                ...
                <p x-show="selectedPayment" x-cloak class="font-semibold text-gray-900"><span x-text="selectedPayment.name"></span></p>
              </div>
            </div>
            <svg x-show="!isPaymentOpen" class="w-5 h-5 text-gray-400" viewBox="0 0 24 24"><path d="m19.5 8.25-7.5 7.5-7.5-7.5" stroke="currentColor" fill="none" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
            <svg x-show="isPaymentOpen" x-cloak class="w-5 h-5 text-gray-400" viewBox="0 0 24 24"><path d="m4.5 15.75 7.5-7.5 7.5 7.5" stroke="currentColor" fill="none" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
          </div>

          <div x-show="isPaymentOpen" x-cloak @click.outside="isPaymentOpen=false" x-transition
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              <template x-for="method in paymentMethods.filter(p=>p.category==='va')" :key="method.id">
                <div @click="selectPayment(method)"
                  ...
                  <p class="font-semibold text-gray-900"><span x-text="method.name"></span></p>
        ...
        <div class="mt-8">
          <label for="terms" class="flex items-start">
            <input id="terms" name="terms" type="checkbox" class="h-5 w-5 rounded border-gray-300 text-indigo-700 focus:ring-indigo-600 mt-0.5" x-model="termsAccepted">
            <span class="ml-3 text-sm text-gray-600">
              Saya setuju dengan <a href="#" class="font-semibold text-indigo-700 hover:underline">syarat & ketentuan</a> dan <a href="#" class="font-semibold text-indigo-700 hover:underline">kebijakan privasi</a>.
            </span>
          </label>
        </div>
    ...
    <div class="lg:col-span-2 mt-12 lg:mt-0">
      <div class="bg-gray-50 border border-gray-200 rounded-lg p-6 sticky top-8">
          ...
          <div class="flex justify-between items-start py-4">
            <div>
              <span x-text="item.name==='Dewasa'?'Dewasa':'Anak-Anak'"
                    ...
                    class="inline-block text-xs font-semibold px-2.5 py-0.5 rounded-full mb-2"></span>
              <p class="font-semibold">Tiket Masuk Watersplash Park</p>
              <p class="text-sm text-gray-600"><span x-text="order.tanggalFormatted"></span>, Total: <span x-text="item.quantity"></span></p>
            </div>
        ...
        <template x-if="order.items.length===0">
          <div class="py-4 text-center text-gray-500"><p>Tidak ada item dalam pesanan Anda.</p></div>
        </template>
...
<button class="fixed bottom-8 right-8 bg-gray-900 text-white w-14 h-14 rounded-full flex items-center justify-center shadow-lg hover:bg-gray-700 transition-colors">
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-7 h-7" fill="none">
...
function pemesananApp(){
  return {
    termsAccepted:false,
    order:{items:[],tanggalFormatted:'Tidak Ditemukan',subtotal:0,biayaAplikasi:0,Total:0},
    isPaymentOpen:false,
    selectedPayment:null,
    paymentMethods:[
      {id:'bca_va',name:'BCA VA',description:'Hanya pembayaran dari Bank BCA',logoUrl:'https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEi1MkMOsEjUfFY7Jthedj-LSUGzQjsNdK0lMCDXw3DO8yaiBSeoFAA19-ZMhZ_6fnefUSRpRY_KP-Ipxh4ZU4x7grVeoLgxBXm8qdIl64U06LMLHaBftYakqllTBPIFLYP3ELbA2lH6_Og/w320-h118/Logo+Bank+BCA.png',category:'va'},
      {id:'bni_va',name:'BNI VA',description:'Menerima semua bank',logoUrl:'https://vectorez.biz.id/wp-content/uploads/2023/10/Logo-Bank-Negara-Indonesia-BNI.png',category:'va'},
      {id:'bri_va',name:'BRI VA',description:'Menerima semua bank',logoUrl:'https://www.freelogovectors.net/wp-content/uploads/2023/02/bri-logo-freelogovectors.net_.png',category:'va'},
      {id:'mandiri_va',name:'Mandiri VA',description:'Hanya dari Bank Mandiri',logoUrl:'https://upload.wikimedia.org/wikipedia/commons/thumb/a/ad/Bank_Mandiri_logo_2016.svg/2560px-Bank_Mandiri_logo.svg.png',category:'va'}
    ],
    loadOrderFromStorage(){
      const s=sessionStorage.getItem('watersplashOrder');
      if(s){
        const d=JSON.parse(s);
        let subtotal=0;
        (d.items||[]).forEach(it=>{subtotal+=it.price*it.quantity;});
        this.order.items=d.items||[];
        this.order.tanggalFormatted=d.tanggalFormatted||'Tidak ditemukan';
        this.order.subtotal=subtotal;
        this.order.biayaAplikasi=0;
        this.order.Total=subtotal;
      }
    },
    formatCurrency(v){ if(isNaN(v)) v=0; return new Intl.NumberFormat('id-ID',{style:'currency',currency:'IDR',minimumFractionDigits:0,maximumFractionDigits:0}).format(v); },
    selectPayment(m){ this.selectedPayment=m; this.isPaymentOpen=false; },
    async submitPayment(){
      const payload={
        ...
        orderItems:this.order.items.map(i=>({id:i.id,quantity:i.quantity,fastTrack:!!i.fastTrack})),
        orderTotal:this.order.Total,
        tanggalFormatted:this.order.tanggalFormatted
      };
      if(!payload.paymentMethod){ alert('Silakan pilih metode pembayaran.'); return; }
      if(!payload.nama||!payload.email||!payload.telepon){ alert('Mohon lengkapi Nama, Email, dan Nomor Telepon.'); return; }

      try{
        const res=await fetch("{{ route('payment.store') }}",{
          method:'POST',
        ...
        if(!res.ok){
          ...
          alert(err.message||'Gagal memproses pembayaran'); return;
        }
        const data=await res.json();
        window.location.href=data.redirect; // /pembayaran/{order_code}
      }catch(e){
